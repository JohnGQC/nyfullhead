...
class RGBNet(nn.Module):
    ...
    def forward(self, input_h, input_views):
        ...
        return outputs
class VolumeFeature(nn.module):
    def __init__(volumn_dimensions,voexls_dimension,feature_channels):
        self.volumn_dimensions= volumn_dimension
        self.num_voexls = voxels_dimension
        self.feature_channels = feature_channels
        self.feature= torch.nn.Parameter(torch.zeros([1, self.voxel_dim, *self.world_size],dtype=torch.float32))
    
'''Model'''
class ManVatar(torch.nn.Module):
